...
export const IntroPresentation = ({ onEnter }: IntroPresentationProps) => {
  ...
  return (
    <main className="intro-presentation relative h-screen overflow-y-auto overflow-x-hidden bg-[#120d0a] text-white">
      <img
        src={capaUrl}
        alt=""
      ...
      <section className="intro-content relative z-10 flex min-h-screen items-end px-5 py-7 sm:px-8 sm:py-10 lg:items-center lg:px-14">
        <div className="intro-shell w-full max-w-5xl">
          <div className="intro-copy max-w-3xl">
            <div className="intro-eyebrow mb-4 inline-flex items-center gap-2 rounded-full border border-amber-200/25 bg-black/35 px-3 py-1.5 text-[11px] uppercase tracking-[0.28em] text-amber-200 backdrop-blur-md">
              <Mountain className="h-3.5 w-3.5" />
              Web art generativa
            </div>
            <h1 className="intro-title max-w-2xl text-[clamp(3rem,8vw,7.4rem)] font-semibold leading-[0.88] text-white">
              Serra Sem Ar
            </h1>
            <p className="intro-lede mt-5 max-w-2xl text-base leading-relaxed text-white/78 sm:text-lg">
          ...
          <div className="intro-metrics mt-8 grid max-w-3xl grid-cols-2 gap-2.5 sm:grid-cols-4">
          ...
          <div className="intro-actions mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
            <Button
              onClick={onEnter}
              className="intro-cta h-12 w-full rounded-lg bg-amber-400 px-6 text-sm font-semibold text-black shadow-2xl shadow-black/30 hover:bg-amber-300 sm:w-auto"
            >
              <Play className="h-4 w-4" />
              Entrar na serra
            </Button>
            <p className="intro-note max-w-sm text-xs leading-relaxed text-white/55">
              Dados históricos do Ministério da Saúde atravessados por registros, memoriais e fragmentos visuais.
            </p>
          </div>
        </div>
      </section>
...
const IntroMetric = ({
  ...
  <div className="intro-metric min-h-[88px] rounded-lg border border-white/12 bg-black/42 p-3 text-white shadow-xl backdrop-blur-md">
    <div className="intro-metric-label mb-3 flex items-center gap-2 text-amber-200/90">
      <Icon className="h-4 w-4" />
      <span className="text-[10px] uppercase tracking-[0.22em]">{label}</span>
    </div>
    <p className="intro-metric-value text-sm font-semibold leading-snug text-white sm:text-base">{value}</p>
  </div>
);
